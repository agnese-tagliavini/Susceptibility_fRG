...
inline int Matidx(int rx, int ry, int rz){
   return (rz+10*FFT_DIM) % FFT_DIM+FFT_DIM*((ry+10*FFT_DIM) % FFT_DIM+ FFT_DIM*((rx+10*FFT_DIM) % FFT_DIM));
}

inline int Matidxshift(int rx, int ry, int rz){
   return (rz+FFT_DIM/2 + 10*FFT_DIM) % FFT_DIM+FFT_DIM*((ry+FFT_DIM/2 + 10*FFT_DIM) % FFT_DIM + FFT_DIM*((rx+FFT_DIM/2 + 10*FFT_DIM) % FFT_DIM));
