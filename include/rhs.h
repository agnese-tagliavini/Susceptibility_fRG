...
class rhs_t 		///< Functor to specify the rhs calculation for both the self-energy and the vertex
{
   public:
      ...
      /********************* RHS evaluation ********************/
      static dcomplex eval_rhs_Sig( const idx_1p_t& se_idx, const state_t& state_vec, const gf_1p_mat_t& Gvec ); 					///< Calculate rhs of self-energy flow for specific index
      static dcomplex eval_diag_suscept_sc( const idx_suscept_t& idx, const state_t& state_vec, const gf_bubble_mat_t& bubble_pp );		///< Calculate particle-particle diagram for the flow
   ...
   private:
      ...
      static symmetry_grp_t<dcomplex,8> symm_grp_chi_sc; 
