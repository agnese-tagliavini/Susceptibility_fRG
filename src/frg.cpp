...
const int POS_FERM_P_IN = 4;
const int POS_BOS_P_IN = 6;

const int POS_BOS_CHI_IN = 64;

const int POS_SIG_IN = 4; 
...
{
   using namespace H5;
   H5File input_file( "dat/dat_U1_Beta5_PFCB64_HUB_INTFL_MKP1_allsymm_projsymm_withSE_speedup_2loop_DIVERGENT.h5", H5F_ACC_RDONLY );
   cout << "Got file" << endl;
...
MatQN G( double w, double kx, double ky,double kz, const MatQN& selfEn )
{
   return ( G0inv( w, kx, ky, kz ) -  selfEn ).inverse();
}
...
MatQN G0inv( double w, double kx, double ky, double kz )
{
   ...
   MatQN G0inv; 
   G0inv <<		
      I*w + 2.0 * ( cos_kx + cos_ky + cos_kz ) + 4.0 * (cos_kx * cos_ky + cos_ky * cos_kz + cos_kx * cos_kz) * T_PRIME - B - MU;
