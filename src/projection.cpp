...
dcomplex generate_proj_matrix_ph_to_pp( const idx_proj_matrix_t& idx  )
{
   ...
   int Qz_idx = std::get<2>(K_Grid::get_indices(Q));  
   ...
   for(int r1 = 0; r1< REAL_GRID_FF_SHELL;++r1){
	...
	int weight_z = (1 + (translate_2pi_z(mp) * translate_2pi_z(np) * (1 - 2 *((r1z+r2z)%2)))); 
	int weight = weight_x * weight_y * weight_z / 8;
	if(weight==1){
	for(int r3x = - MAX_PROJ_R_GRID+abs((r1x+r2x)%2); r3x< MAX_PROJ_R_GRID+1 -abs((r1x+r2x)%2) ; r3x+=2)
	      ...
	      for(int r3z = - MAX_PROJ_R_GRID+abs((r1y+r2y)%2) ; r3z< MAX_PROJ_R_GRID+1-abs((r1y+r2y)%2) ; r3z+=2){
        
   		 if( abs( r1x+r2x-r3x ) <= (FFREAL_DIM-1) && abs( r1y+r2y-r3y ) <= (FFREAL_DIM-1) && abs( r1z+r2z-r3z ) <= (FFREAL_DIM-1))
   		    if( abs(-r1x-r2x-r3x ) <= (FFREAL_DIM-1) && abs(-r1y-r2y-r3y ) <= (FFREAL_DIM-1) && abs(-r1z-r2z-r3z ) <= (FFREAL_DIM-1)){
   		       val+= conj(ffactor_real(m,(r1x+r2x-r3x)/2 ,(r1y+r2y-r3y)/2, (r1z+r2z-r3z)/2))*
			     ffactor_real(n,-(r1x+r2x+r3x)/2 ,-(r1y+r2y+r3y)/2, -(r1z+r2z+r3z)/2)*
			     ffactor_real(mp, r1x ,r1y, r1z)*conj(ffactor_real(np, r2x ,r2y, r2z))*
			     (cos_k(Qx_idx * r3x + Qy_idx * r3y + Qz_idx * r3z + Kx_idx * (r2x-r1x) + Ky_idx * (r2y-r1y)+ Kz_idx * (r2z-r1z)) + I * sin_k(Qx_idx * r3x + Qy_idx * r3y + Qz_idx * r3z + Kx_idx * (r2x-r1x) + Ky_idx * (r2y-r1y)+Kz_idx * (r2z-r1z) ));
		 }
	}
	}
	}
