...
operation hmirror_phi_pp( idx_phi_t& idx )
{
   ...
   bf_sign_y &= bool(bf[1]);
   bf_sign_z &= bool(bf[2]);
   return operation( sign^(bf_sign_x^bf_sign_y^bf_sign_z) , false ); 	// true=1 is multiplying a factor -1 to the matrix element
...
operation cconj_phi_pp( idx_phi_t& idx ) //notation changed but fine = cconj*hmirror in 3D_patch
{
   ...
   bf_sign_y &= bool(bf[1]);
   bf_sign_z &= bool(bf[2]);
   return operation( sign^(bf_sign_x^bf_sign_y^bf_sign_z) , true );
}
...
operation cconj_timerev_P_pp( idx_P_t& idx )
{
   ...
   bf_sign_y &= bool(bf[1]);
   bf_sign_z &= bool(bf[2]);
   return operation( sign^(bf_sign_x^bf_sign_y^bf_sign_z) , true ); // Quantum numbers don't change after cconj and time reversal
}
...
operation hmirror_timerev_bubble_ph( idx_bubble_mat_t& idx)
{
      idx(0) *= -1;
      idx(1) *=  1;
      swap(idx(4),idx(7));
